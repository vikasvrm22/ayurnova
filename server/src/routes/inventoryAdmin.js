...
import { validateBatch, sanitizeText, isValidUUID } from "../validation/validators.js";
...
router.get("/batches", requireStaffAuth, async (req, res, next) => {
  try {
    let query = supabaseAdmin().from("batches").select("*").order("created_at", { ascending: false });
