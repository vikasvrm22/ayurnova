...
Run via `cd quality-agents/ui-judge && npm test` (9 tests, all passing as of
this writing - 7 covering the audit pipeline as below, plus
`referenceDiscovery` and `referenceInventory` covering filename parsing,
classification, duplicate detection, explicit-config precedence, and safe
route mapping):
